"""
Vim 插件整合測試 - 以 headless vim 實際驅動 plugin/verilog_auto.vim。

自動化先前只能手動跑的端到端流程，並把插件的關鍵約定鎖進 CI：
預設 g:verilog_auto_script 指向真正的 pyvauto.py、:VerilogAuto 命令存在、
...
PLUGIN = REPO / "plugin" / "verilog_auto.vim"
...
def test_plugin_defaults_and_mappings(tmp_path):
    """預設腳本路徑解析到存在的 pyvauto.py，:VerilogAuto 與 \\va 都就緒。"""
    result = tmp_path / "result.txt"
    script = tmp_path / "check.vim"
    script.write_text(
        f"source {PLUGIN}\n"
        f"redir! > {result}\n"
        "echo 'script_readable=' . filereadable(g:verilog_auto_script)\n"
        "echo 'has_VerilogAuto=' . exists(':VerilogAuto')\n"
    ...
    assert "has_VerilogAuto=2" in text           # command is defined
    assert "VerilogAuto" in text                 # \va maps to :VerilogAuto


def test_plugin_end_to_end_expansion(tmp_path):
    """在 vim 內開檔 → :VerilogAuto → 檔案被就地展開（完整端到端）。"""
